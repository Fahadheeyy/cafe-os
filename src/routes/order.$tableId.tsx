...
import { ArrowLeft, Minus, Plus, ShoppingCart, Trash2, Printer, X, UtensilsCrossed, Loader2 } from "lucide-react";
...
function OrderScreen() {
  ...
  const payAndPrint = async () => {
    ...
    try {
      ...
      navigate({ to: homePath });
    } catch (err) { toast.error(err instanceof Error ? err.message : "Payment or print failed"); }
  };
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col">
      <header className="h-16 border-b bg-background/80 backdrop-blur sticky top-0 z-30 flex items-center px-4 gap-3">
        <Button variant="ghost" size="icon" aria-label="Back" onClick={() => navigate({ to: homePath })}><ArrowLeft className="h-5 w-5" /></Button>
        ...
        <button onClick={() => setCartOpen(true)} className="lg:hidden relative rounded-xl border px-3 py-2 flex items-center gap-2 active:scale-95 transition">
          ...
          {totalQty > 0 && <span className="absolute -top-1.5 -right-1.5 h-5 min-w-5 px-1 rounded-full bg-primary text-primary-foreground text-[10px] font-semibold grid place-items-center">{totalQty}</span>}
        </button>
      </header>
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
              {CATEGORIES.map((c) => (
                <button
                  key={c}
                  onClick={() => setCat(c)}
                  className={`px-4 h-9 rounded-full text-sm font-medium whitespace-nowrap transition ${cat === c ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground hover:bg-accent"}`}
                >
                  {c}
                </button>
              ))}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="h-9 w-9 rounded-full bg-primary text-primary-foreground grid place-items-center"><Plus className="h-4 w-4" /></div>
                      </div>
                    </button>
                  );
                })}
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
      ...
      <div className="flex-1 overflow-y-auto">
        {cart.length === 0 ? (
          ...
          <ul className="p-3 space-y-2">
            {cart.map((i) => (
              <li key={i.productId ?? i.name} className="p-3 rounded-xl border">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate">{i.name}</p>
                    <p className="text-xs text-muted-foreground">{money(i.price, currency)} each</p>
                  </div>
                  <button onClick={() => i.productId && onRemove(i.productId)} className="text-muted-foreground hover:text-destructive p-1" aria-label={`Remove ${i.name}`}>
                    <Trash2 className="h-4 w-4" />
                  </button>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2">
                    <button onClick={() => i.productId && onDec(i.productId)} className="h-9 w-9 rounded-lg border grid place-items-center hover:bg-accent active:scale-95 transition" aria-label={`Decrease ${i.name}`}><Minus className="h-3.5 w-3.5" /></button>
                    <span className="w-6 text-center text-sm font-medium" aria-live="polite">{i.qty}</span>
                    <button onClick={() => i.productId && onAdd(i.productId)} className="h-9 w-9 rounded-lg border grid place-items-center hover:bg-accent active:scale-95 transition" aria-label={`Increase ${i.name}`}><Plus className="h-3.5 w-3.5" /></button>
                  </div>
      ...
      <div className="border-t p-4 space-y-3 bg-card">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["upi", "cash"] as const).map((m) => (
              <button
                key={m}
                onClick={() => onMethod(m)}
                className={`h-10 rounded-xl border text-sm font-medium transition ${method === m ? "bg-primary text-primary-foreground border-primary shadow-sm" : "bg-background hover:bg-accent"}`}
              >
                {m === "upi" ? "UPI" : "Cash"}
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={onSave} disabled={busy}>Save order</Button>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="ghost" size="sm" onClick={onPrint} disabled={busy}><Printer className="h-4 w-4 mr-1" /> Print bill</Button>
