...
BEGIN
  ...
  ELSE
    INSERT INTO public.orders (business_id, table_id, table_name, total, staff_id, staff_name, order_type, parcel_fee, notes)
    VALUES (bid, _table_id, COALESCE(t_name, 'Takeaway'), total_amount, caller, COALESCE(caller_name, 'Staff'), _order_type, _parcel_fee, _notes)
  ...
  -- 3. Loop through incoming items to diff quantities & update notes
  FOR item IN 
    SELECT 
  ...
  LOOP
    ...
    IF delta > 0 THEN
      ...
      INSERT INTO public.order_items (business_id, order_id, kot_id, product_id, name, price, qty, notes)
      VALUES (bid, target_order_id, new_kot_id, item.product_id, item.name, item.price, delta, item.notes);
    ...
    ELSIF delta < 0 THEN
      ...
      LOOP
        ...
        IF rec.qty <= delta THEN
          ...
          delta := delta - rec.qty;
        ELSE
          UPDATE public.order_items SET qty = qty - delta, notes = item.notes WHERE id = rec.id;
          delta := 0;
        END IF;
      END LOOP;
    ELSE
      -- Delta is 0, update item notes if changed
      UPDATE public.order_items 
      SET notes = item.notes 
      WHERE order_id = target_order_id 
        AND (product_id = item.product_id OR (product_id IS NULL AND item.product_id IS NULL AND name = item.name));
    END IF;
  END LOOP;
