CREATE TABLE IF NOT EXISTS public.kots (
  ...
  kitchen_status public.kitchen_status DEFAULT 'queued',
...
CREATE POLICY "Users can manage their business kots" ON public.kots
  FOR ALL USING (business_id = (SELECT business_id FROM public.profiles WHERE id = auth.uid()));

-- Add kot_id to order_items
ALTER TABLE public.order_items ADD COLUMN kot_id uuid REFERENCES public.kots(id) ON DELETE CASCADE;

-- Update upsert_order_with_items for KOT diffing
...
BEGIN
  ...
  -- 1. Create a temp table of incoming items aggregated by product_id/name
  CREATE TEMP TABLE incoming_items ON COMMIT DROP AS
  SELECT 
  ...
  -- 2. Find items in the DB that are NOT in incoming_items and delete them
  DELETE FROM public.order_items oi
  WHERE oi.order_id = target_order_id
    AND NOT EXISTS (
      SELECT 1 FROM incoming_items ii 
      WHERE (ii.product_id = oi.product_id OR (ii.product_id IS NULL AND oi.product_id IS NULL AND ii.name = oi.name))
    );

  -- 3. Loop through incoming items to diff quantities
  FOR item IN SELECT * FROM incoming_items LOOP
